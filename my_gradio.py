...
# from models.spatial_transformers.spatial_transformer import (
#     ComposedSTN, SpatialTransformer, unravel_index)
from PIL import Image
from torchvision.utils import make_grid, save_image
from utils.download import download_model
from utils.vis_tools.helpers import load_pil, normalize, pil_to_tensor
...
def init_images(global_state):
    ...
    state['mask'] = np.ones((init_image.size[1], init_image.size[0]),
                                dtype=np.uint8)
    state['images']['imageB_show'] = state['renderer']._apply_delta(state['generator_params'], #res
                            None, #new_ws
                            state['params']['seed']+1,
                            'const', #noise_mode
                            state['params']['trunc_psi'],  # trunc_psi,
                            state['params']['trunc_cutoff'],  # trunc_cutoff,
                            to_pil=True)
    state['images']['imageC_show'] = state['renderer']._apply_delta(state['generator_params'], #res
                                None, #new_ws
                                state['params']['seed']+2,
                                'const', #noise_mode
                                state['params']['trunc_psi'],  # trunc_psi,
                                state['params']['trunc_cutoff'],  # trunc_cutoff,
                                to_pil=True)
    state['images']['imageD_show'] = state['renderer']._apply_delta(state['generator_params'], #res
                                None, #new_ws
                                state['params']['seed']+3,
                                'const', #noise_mode
                                state['params']['trunc_psi'],  # trunc_psi,
                                state['params']['trunc_cutoff'],  # trunc_cutoff,
                                to_pil=True)
    state['images']['imageE_show'] = state['renderer']._apply_delta(state['generator_params'], #res
                                    None, #new_ws
                                    state['params']['seed']+4,
                                    'const', #noise_mode
                                    state['params']['trunc_psi'],  # trunc_psi,
                                    state['params']['trunc_cutoff'],  # trunc_cutoff,
                                    to_pil=True)
    state['images']['imageF_show'] = state['renderer']._apply_delta(state['generator_params'], #res
                                    None, #new_ws
                                    state['params']['seed']+5,
                                    'const', #noise_mode
                                    state['params']['trunc_psi'],  # trunc_psi,
                                    state['params']['trunc_cutoff'],  # trunc_cutoff,
                                    to_pil=True)
    state['generator_params'].delta_ws = torch.zeros(state['generator_params'].w.shape)
    state['generator_params'].delta_w_star = torch.zeros(state['generator_params'].w.shape)
...
with gr.Blocks() as demo:
    ...
    # flow_size = 128
    # stn = get_stn(['similarity', 'flow'], flow_size=flow_size, supersize=resolution).to('cuda')  # instantiate STN
    # stn.load_state_dict(ckpt['t_ema'])

    ### LOAD GANGEALING END

    # cache_dir = './checkpoints'#args.cache_dir
    cache_dir = './scripts/checkpoints'#args.cache_dir
    ...
    init_pkl = 'stylegan2-afhqcat-512x512'
    # renderer = Renderer()
    global_state = gr.State({
        ...
        # "renderer2": Renderer(disable_timing=Trdefue),
        # "stn": stn,
        "points": {},
        "curr_point": None,
    ...
    with gr.Row():
        with gr.Row():
            ...
            with gr.Column(scale=3):
                ...
                # with gr.Row():
                #     with gr.Column(scale=1, min_width=10):
                #         gr.Markdown(value='Adjust Alpha', show_label=False)
                #     with gr.Column(scale=4, min_width=10):
                #         form_alpha_number = gr.Slider(minimum=0,
                #             maximum=2, step=0.1,
                #             value=1,
                #             label='Alpha',
                #             interactive=True)
                with gr.Row():
                    with gr.Column(scale=1, min_width=10):
                        gr.Markdown(value='Calibrate', show_label=False)
                    # with gr.Column(scale=1, min_width=10):
                    #     # form_optimize_delta_w = gr.Button('Optimize')
                    #     form_update_delta_w = gr.Button('Update')
                    with gr.Column(scale=1, min_width=10):
                        form_optimize_delta_w = gr.Button('Optimize')
                        # form_optimize_delta_w = gr.Button('Update')
                    with gr.Column(scale=1, min_width=10):
                        form_apply_to_all = gr.Button('Apply')
                with gr.Row():
                    with gr.Column(scale=1, min_width=10):
                        gr.Markdown(value='Alpha*', show_label=False)
                    with gr.Column(scale=4, min_width=10):
                        form_alpha_number_star = gr.Slider(minimum=-50,
                            maximum=50, step=0.1,
                            value=1,
                            label='Alpha*',
                            interactive=True)
            ...
            # Right --> Image
            with gr.Column(scale=4):
                second_image = ImageMask(
                    value= global_state.value['images']['imageB_show'],
                    brush_radius=20).style(
                        width=512,
                        height=512)  # NOTE: hard image size code here.
    with gr.Row():
         with gr.Column(scale=1):
            third_image = ImageMask(
                value= global_state.value['images']['imageC_show'],
                brush_radius=20).style(
                width=512,
                height=512)
         with gr.Column(scale=1):
            forth_image = ImageMask(
                value= global_state.value['images']['imageD_show'],
                brush_radius=20).style(
                width=512,
                height=512)
         with gr.Column(scale=1):
            fifth_image = ImageMask(
                value= global_state.value['images']['imageE_show'],
                brush_radius=20).style(
                width=512,
                height=512)
         with gr.Column(scale=1):
            sixth_image = ImageMask(
                value= global_state.value['images']['imageF_show'],
                brush_radius=20).style(
                width=512,
                height=512)
    ...
    def on_change_pretrained_dropdown(pretrained_value, global_state):
        ...
        return global_state, global_state["images"]['image_show'], global_state["images"]['imageB_show'], global_state["images"]['imageC_show'], global_state["images"]['imageD_show'], global_state["images"]['imageE_show'], global_state["images"]['imageF_show'], global_state['params']['alpha*']

    form_pretrained_dropdown.change(
        on_change_pretrained_dropdown,
        inputs=[form_pretrained_dropdown, global_state],
        outputs=[global_state, form_image, second_image, third_image, forth_image, fifth_image, sixth_image, form_alpha_number_star],
    )
    ...
    def on_click_reset_image(global_state):
        ...
        return global_state, global_state['images']['image_show'], global_state['images']['imageB_show'], global_state['images']['imageC_show'], global_state['images']['imageD_show'], global_state['images']['imageE_show'], global_state['images']['imageF_show']

    form_reset_image.click(
        on_click_reset_image,
        inputs=[global_state],
        outputs=[global_state, form_image, second_image, third_image, forth_image, fifth_image, sixth_image],
    )
    ...
    def on_change_update_image_seed(seed, global_state):
        ...
        return global_state, global_state['images']['image_show'], global_state['images']['imageB_show']

    form_seed_number.change(
        on_change_update_image_seed,
        inputs=[form_seed_number, global_state],
        outputs=[global_state, form_image, second_image],
    )
    ...
    def on_change_lr(lr, global_state):
        if lr == 0:
            ...
            # renderer2 = global_state['renderer2']
            # renderer2.update_lr(lr)
            print('New optimizer: ')
    ...
    def on_click_start(global_state, image):
        ...
        if len(global_state["points"]) == 0:
            ...
            # renderer2: Renderer = global_state["renderer2"]
            ...
            while True:
                ...
                renderer._render_drag_impl(
                    ...
                    is_drag=True,
                    to_pil=True)
                # print(f'Drag time: {time.time() - start_time}')
                # print(f'Apply delta time: {time.time() - start_time}')
                start_time = time.time()
                if step_idx % global_state['draw_interval'] == 0:
                    ...
                    global_state['images']['imageB_show'] = renderer._apply_delta(global_state['generator_params'], #res
                            global_state['generator_params'].delta_ws, #new_ws
                            global_state['params']['seed']+1,
                            'const', #noise_mode
                            global_state['params']['trunc_psi'],  # trunc_psi,
                            global_state['params']['trunc_cutoff'],  # trunc_cutoff,
                            to_pil=True)
                    
                    global_state['images']['imageC_show'] = renderer._apply_delta(global_state['generator_params'], #res
                            global_state['generator_params'].delta_ws, #new_ws
                            global_state['params']['seed']+2,
                            'const', #noise_mode
                            global_state['params']['trunc_psi'],  # trunc_psi,
                            global_state['params']['trunc_cutoff'],  # trunc_cutoff,
                            to_pil=True)
                    global_state['images']['imageD_show'] = renderer._apply_delta(global_state['generator_params'], #res
                            global_state['generator_params'].delta_ws, #new_ws
                            global_state['params']['seed']+3,
                            'const', #noise_mode
                            global_state['params']['trunc_psi'],  # trunc_psi,
                            global_state['params']['trunc_cutoff'],  # trunc_cutoff,
                            to_pil=True)
                    global_state['images']['imageE_show'] = renderer._apply_delta(global_state['generator_params'], #res
                            global_state['generator_params'].delta_ws, #new_ws
                            global_state['params']['seed']+4,
                            'const', #noise_mode
                            global_state['params']['trunc_psi'],  # trunc_psi,
                            global_state['params']['trunc_cutoff'],  # trunc_cutoff,
                            to_pil=True)
                    global_state['images']['imageF_show'] = renderer._apply_delta(global_state['generator_params'], #res
                            global_state['generator_params'].delta_ws, #new_ws
                            global_state['params']['seed']+5,
                            'const', #noise_mode
                            global_state['params']['trunc_psi'],  # trunc_psi,
                            global_state['params']['trunc_cutoff'],  # trunc_cutoff,
                            to_pil=True)
                    # print('Apply delta time: ', time.time() - start_time)
                    # save_location = './pilot-study/{}/{}'.format(global_state['pretrained_weight'], global_state['params']['seed'])
                    # os.makedirs(save_location, exist_ok=True)
                    # torch.save(global_state['generator_params'].delta_ws, os.path.join(save_location, f'{step_idx}.pt'))
                    # global_state['generator_params'].delta_ws
                    # with open(os.path.join(save_location, 'distance.txt'), 'a+') as the_file:
                    #     the_file.write('{}: {}\n'.format(step_idx, global_state['generator_params'].distance_in_pixel.item()))
                    yield (
                        global_state,
                        step_idx,
                        global_state['images']['image_show'],
                        global_state['images']['imageB_show'],
                        global_state['images']['imageC_show'],
                        global_state['images']['imageD_show'],
                        global_state['images']['imageE_show'],
                        global_state['images']['imageF_show'],
                        # gr.File.update(visible=False),
                        gr.Button.update(interactive=False),
                        gr.Button.update(interactive=False),
                        gr.Button.update(interactive=False),
    ...
    def on_click_alpha_star(alpha, global_state):
        global_state['params']['alpha*'] = alpha
        print(alpha)
        renderer: Renderer = global_state["renderer"]
        global_state['images']['image_show'] = renderer._apply_delta_2(global_state['generator_params'], #res
                global_state['generator_params'].delta_w_star*alpha, #new_ws
        ...
        global_state['images']['imageB_show'] = renderer._apply_delta_2(global_state['generator_params'], #res
                global_state['generator_params'].delta_w_star*alpha, #new_ws
                global_state['params']['seed']+1,
                # alpha,
                'const', #noise_mode
        ...
        global_state['images']['imageC_show'] = renderer._apply_delta_2(global_state['generator_params'], #res
                global_state['generator_params'].delta_w_star*alpha, #new_ws
                global_state['params']['seed']+2,
                # alpha,
                'const', #noise_mode
        ...
        global_state['images']['imageD_show'] = renderer._apply_delta_2(global_state['generator_params'], #res
                global_state['generator_params'].delta_w_star*alpha, #new_ws
                global_state['params']['seed']+3,
                # alpha,
                'const', #noise_mode
        ...
        global_state['images']['imageE_show'] = renderer._apply_delta_2(global_state['generator_params'], #res
                global_state['generator_params'].delta_w_star*alpha, #new_ws
                global_state['params']['seed']+4,
                # alpha,
                'const', #noise_mode
        ...
        global_state['images']['imageF_show'] = renderer._apply_delta_2(global_state['generator_params'], #res
                global_state['generator_params'].delta_w_star*alpha, #new_ws
                global_state['params']['seed']+5,
                # alpha,
                'const', #noise_mode
    ...
    form_alpha_number_star.change(
        on_click_alpha_star,
        inputs=[form_alpha_number_star, global_state],
        outputs=[
            global_state,
            form_steps_number,
            form_image,
    ...
    def optimize_delta_w(global_state):
        renderer: Renderer = global_state["renderer"]
        # delta_w_star = renderer.optimize_delta_w_2(global_state['generator_params'], #res
        delta_w_star = renderer.optimize_delta_w_batch(global_state['generator_params'], #res
                                                global_state['generator_params'].delta_ws, #new_ws
                                                global_state['params']['seed'],
                                                # global_state['params']['alpha'],
                                                'const', #noise_mode
                                                ...
                                                to_pil=True,
                                                log_tb=False)
        # delta_w_star = renderer.update_delta_w(global_state['generator_params'], #res
        #                                         global_state['generator_params'].delta_ws, #new_ws
        #                                         global_state['params']['seed'],
        #                                         global_state['params']['alpha'],
        #                                         'const', #noise_mode
    ...
    def on_click_save(global_state):
        ...
        save_location = os.path.join('./delta_ws/{}'.format(global_state['pretrained_weight']))
        os.makedirs(save_location, exist_ok=True)
        save_location = os.path.join(save_location, '{}.pt'.format(global_state['params']['seed']))
        torch.save(global_state['generator_params'].delta_ws, save_location)
        print('Saved in: ', save_location)

        return global_state, gr.Button.update(interactive=False)

    form_save_btn.click(on_click_save,
                        inputs=[global_state],
...
if __name__ == "__main__":
    ...
    demo.launch()
    # demo.launch(share=args.share, server_name="0.0.0.0" if args.listen else "127.0.0.1")
